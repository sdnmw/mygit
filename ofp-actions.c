...
/* ## ----------------- ## */
    ...
    /* OF1.0(28): void. */
    OFPAT_RAW10_CAR_MOVE, 
    /* OF1.1(28): void. */
    OFPAT_RAW11_CAR_MOVE,
    /* OF1.2-1.4(28): void. */
    OFPAT_RAW12_CAR_MOVE,

    /* OF1.0(29): void. */
    OFPAT_RAW10_PICAMERA, 
    /* OF1.1(29): void. */
    OFPAT_RAW11_PICAMERA,
    /* OF1.2-1.4(29): void. */
    OFPAT_RAW12_PICAMERA,
...
ofpact_next_flattened(const struct ofpact *ofpact)
{
    ...
    case OFPACT_CAR_MOVE:
    case OFPACT_PICAMERA:
...
static enum ofperr
decode_OFPAT_RAW10_CAR_MOVE(struct ofpbuf *out)
{
    ofpact_put_CAR_MOVE(out)->ofpact.raw = OFPAT_RAW10_CAR_MOVE;
    return 0;
}
 
static enum ofperr
decode_OFPAT_RAW11_CAR_MOVE(struct ofpbuf *out)
{
    ofpact_put_CAR_MOVE(out)->ofpact.raw = OFPAT_RAW11_CAR_MOVE;
    return 0;
}
 

static enum ofperr
decode_OFPAT_RAW12_CAR_MOVE(struct ofpbuf *out)	
{
    ofpact_put_CAR_MOVE(out)->ofpact.raw = OFPAT_RAW10_CAR_MOVE;
    return 0;
}
...
static void
encode_CAR_MOVE(const struct ofpact_null *null OVS_UNUSED,
                  enum ofp_version ofp_version, struct ofpbuf *out)
{
    if (ofp_version == OFP10_VERSION) {
        put_OFPAT10_CAR_MOVE(out);
    } else if (ofp_version == OFP11_VERSION) {
        put_OFPAT11_CAR_MOVE(out);
    } else {
        put_OFPAT12_CAR_MOVE(out);
    }
}
 
static char * OVS_WARN_UNUSED_RESULT
parse_CAR_MOVE(char *arg OVS_UNUSED, struct ofpbuf *ofpacts,
                 enum ofputil_protocol *usable_protocols OVS_UNUSED)
{
    ofpact_put_CAR_MOVE(ofpacts)->ofpact.raw = OFPAT_RAW10_CAR_MOVE;
    return NULL;
}
 
static void
format_CAR_MOVE(const struct ofpact_null *a OVS_UNUSED, struct ds *s)
{
    ds_put_cstr(s, "car_move");
}


static enum ofperr
decode_OFPAT_RAW10_PICAMERA(struct ofpbuf *out)
{
    ofpact_put_PICAMERA(out)->ofpact.raw = OFPAT_RAW10_PICAMERA;
    return 0;
}
 
static enum ofperr
decode_OFPAT_RAW11_PICAMERA(struct ofpbuf *out)
{
    ofpact_put_PICAMERA(out)->ofpact.raw = OFPAT_RAW11_PICAMERA;
    return 0;
}
 

static enum ofperr
decode_OFPAT_RAW12_PICAMERA(struct ofpbuf *out)	
{
    ofpact_put_PICAMERA(out)->ofpact.raw = OFPAT_RAW10_PICAMERA;
    return 0;
}
...
static void
encode_PICAMERA(const struct ofpact_null *null OVS_UNUSED,
                  enum ofp_version ofp_version, struct ofpbuf *out)
{
    if (ofp_version == OFP10_VERSION) {
        put_OFPAT10_PICAMERA(out);
    } else if (ofp_version == OFP11_VERSION) {
        put_OFPAT11_PICAMERA(out);
    } else {
        put_OFPAT12_PICAMERA(out);
    }
}
 
static char * OVS_WARN_UNUSED_RESULT
parse_PICAMERA(char *arg OVS_UNUSED, struct ofpbuf *ofpacts,
                 enum ofputil_protocol *usable_protocols OVS_UNUSED)
{
    ofpact_put_PICAMERA(ofpacts)->ofpact.raw = OFPAT_RAW10_PICAMERA;
    return NULL;
}
 
static void
format_PICAMERA(const struct ofpact_null *a OVS_UNUSED, struct ds *s)
{
    ds_put_cstr(s, "picamera");
}
