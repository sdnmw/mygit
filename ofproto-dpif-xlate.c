...
static void
xlate_car_move_action(struct xlate_ctx *ctx,struct flow *flow)
{
		ctx->xin->flow = *flow;
		VLOG_INFO("OFPACT_CAR_MOVE");
	
}

static void
xlate_picamera_action(struct xlate_ctx *ctx,struct flow *flow)
{
		ctx->xin->flow = *flow;
		VLOG_INFO("OFPACT_PICAMERA");
                 ...
                 struct xlate_ctx *ctx)
{
	...
	case OFPACT_CAR_MOVE:
            CHECK_MPLS_RECIRCULATION();
            xlate_car_move_action(ctx,flow);
            break;

        case OFPACT_PICAMERA:
            CHECK_MPLS_RECIRCULATION();
            xlate_picamera_action(ctx,flow);
            break; 
